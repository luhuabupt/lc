...
func decodeCiphertext(encodedText string, rows int) string {
	...
	for i, idx := 0, 0; i < m; i++ {
		grids[i] = make([]byte, n)
		for j := 0; j < n; j++ {
			grids[i][j] = encodedText[idx]
			idx++
		}
	}

	ans := []byte{}
	for i, j, loop := 0, 0, 0; loop < n; loop++ {
		for i < m && j < n {
			ans = append(ans, grids[i][j])
			i++
			j++
		}
		i = 0
...
func friendRequests_(n int, restrictions [][]int, requests [][]int) []bool {
	forbid := make([]map[int]bool, n)
	sub := make([]map[int]bool, n) // 朋友圈
	father := make([]int, n)       // 圈id
	for i := 0; i < n; i++ {
		father[i] = i
		sub[i] = map[int]bool{i: true}
		forbid[i] = map[int]bool{}
	}
	...
	for _, arr := range requests {
		...
		if forbid[fa][fb] || forbid[fb][fa] {
			ans = append(ans, false)
			continue
		}

		if len(sub[fa]) < len(sub[fb]) {
			fa, fb = fb, fa
		}

		for k, _ := range sub[fb] {
			father[k] = fa
			sub[fa][k] = true
		}

		fbn := map[int]bool{}
		for k, _ := range forbid[fa] {
			fbn[father[k]] = true
		}
		for k, _ := range forbid[fb] {
			fbn[father[k]] = true
		}
		forbid[fa] = fbn

		//fmt.Println(arr, father)
		//fmt.Println(sub)
		//fmt.Println(forbid)
...
func friendRequests(n int, restrictions [][]int, requests [][]int) []bool {
	...
	for _, v := range requests {
		...
		if forbid[a][b] || forbid[b][a] {
			ans = append(ans, false)
			continue
		}

		if len(forbid[a]) < len(forbid[b]) {
			b, a = a, b
		}
		for k, _ := range forbid[b] {
			forbid[a][k] = true
		}
		father[b] = a
