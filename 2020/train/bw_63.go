...
func minMovesToSeat(seats []int, students []int) int {
	...
	ans := 0
	for k, v := range seats {
		ans += abs_(v, students[k])
	}
	return ans
}

func abs_(a, b int) int {
	if a > b {
		return a - b
	}
	return b - a
...
func networkBecomesIdle(edges [][]int, patience []int) int {
	...
	ans := 0
	for i, v := range patience {
		if i == 0 {
			continue
		}
		x := ((2*dp[i]-1)/v)*v + 2*dp[i]
		if x > ans {
			ans = x
		}
	}
